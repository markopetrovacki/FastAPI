...
async def create_roba(roba:  roba_schemas.GetRoba, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):

    new_roba = models.Roba(**roba.dict())
...
def update_roba(id:int, updated_roba: roba_schemas.GetRoba, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
   ...
   #                         detail="Not Authorized to perform requested action")
    ...
    roba_query.update(updated_roba.dict(), synchronize_session=False)
    db.commit() 
